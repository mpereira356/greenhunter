...
def dashboard():
    ...
    ).options(load_only(MatchAlert.status, MatchAlert.stake_amount, MatchAlert.stake_odd)).all()
    ...
    finance_today_alerts = finance_alerts
    today_tickets = finance_tickets
    ...
    monitored_games = db.session.query(func.count(func.distinct(LiveGameState.game_id))).filter(
        LiveGameState.updated_at >= now - timedelta(hours=24)
    ).scalar() or 0
...
def edit_saved_ticket(ticket_id):
    ...
    elif statuses and "pending" not in statuses and "unavailable" in statuses:
...
def save_ticket():
    ...
    for item in items[:100]:
        ...
        if not market_key or (market_key not in {"goal_ht", "over15", "over25", "under15", "under25", "under35", "btts", "btts_yes"} and line is None):
            db.session.rollback()
