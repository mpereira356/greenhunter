...
# A lista diária é compartilhada por todos os usuários. Uma hora evita que a
# navegação comum espere repetidamente a fonte externa; detalhes de mercados
# continuam com seu cache curto e independente.
QUALPLACAR_ODDS_TTL_SECONDS = int(os.environ.get("QUALPLACAR_ODDS_TTL_SECONDS", str(60 * 60)))
...
def _load_detail_cache(game_id: str, allow_stale: bool = False) -> dict | None:
    path = _detail_cache_path(game_id)
    try:
        if not allow_stale and time.time() - os.stat(path).st_mtime > QUALPLACAR_DETAIL_TTL_SECONDS:
            return None
        with open(path, "r", encoding="utf-8") as handle:
            payload = json.load(handle)
    except (OSError, TypeError, ValueError):
...
def _get_detail(game_id: str) -> dict:
    ...
    with _cache_lock:
        ...
        try:
            ...
            # O detalhe pode ficar temporariamente indisponível por DNS ou
            # timeout. Odds antigas são preferíveis a remover todas as cotações
            # do bilhete; a interface volta a atualizá-las na próxima consulta.
            return _load_detail_cache(game_id, allow_stale=True) or {}
        if detail:
            try:
                os.makedirs(QUALPLACAR_ODDS_CACHE_DIR, exist_ok=True)
                path = _detail_cache_path(game_id)
